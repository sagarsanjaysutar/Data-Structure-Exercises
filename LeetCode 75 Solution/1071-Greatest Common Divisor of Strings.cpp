 ...
 * 1. The problem states to return a string. This string represents a "common word" between two input strings such that
 *    concatinating it with itself should result in the input string.
 * 2. e.g. Amongst `ABABAB & ABAB`, AB is a "common word" such that, after concatinating with itself, it results
 *    in ABABAB & ABAB.
 * 3. Note: In this example, ABAB (2nd word) is not common string, because ABAB+ABAB != 1st word (ABABAB).
 *
 * ## What approach solved the problem?
 * @date 15th December 2025
 * 1. The solution involves iterating the smallest word amongst the two.
 * 2. With each iteration, we build a 'divisor' variable consisting of common char. b/w the two words.
 * 3. This 'divisor' variable is checked against the two words to determine if it's common divisor.
 * 4. With each iteration, we end up building largest common-divisor. Thus in the end we find GCD.
 */
#include "../Utils.cpp"
#include <algorithm>
#include <iostream>
